...
public class Store {
	...
	public static int iconTextY = -20; // watchout
	...
	public void click(int mouseButton){
		if(mouseButton == 1)// mouse button == 1 , means left button 
			{
			...
			if(holdsItem){ // makes objects placeable
						...
						for(int x=0;x<Screen.room.block[0].length;x++){
							if(Screen.room.block[y][x].contains(Screen.mse)){
								if(Screen.room.block[y][x].groundID !=Value.groundRoad&& Screen.room.block[y][x].airID == Value.airAir){
									...
									Screen.coinage -=buttonPrice[realID]; // make the object take coins
								}
							}
	...
	public void draw(Graphics g){
		for(int i=0; i < button.length ;i++){
			...
			g.drawImage(Screen.tileset_res[0],button[i].x, button[i].y, button[i].width, button[i].height,null); // Item boxes
		...
		g.setFont(new Font("Courier New", Font.BOLD,14));
		g.setColor(new Color(255,255,255));
		g.drawString(""+ Screen.health,buttonHealth.x + buttonHealth.width+ iconSpace,buttonHealth.x + buttonHealth.y+iconTextY);
		g.drawString(""+ Screen.coinage,buttonCoins.x + buttonCoins.width+ iconSpace,buttonCoins.x + buttonCoins.y+iconTextY);
